...
from examples.flow_builder.document_processing.tools.document_processing_flow import build_document_processing_flow
...
async def main():
    ...
    my_flow_definition = await build_document_processing_flow().compile_deploy()
    generated_folder = f"{Path(__file__).resolve().parent}/generated"
    my_flow_definition.dump_spec(f"{generated_folder}/docprocessing_flow.json")
    global flow_run
